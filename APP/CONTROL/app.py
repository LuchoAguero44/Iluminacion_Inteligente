...
porcentaje_luz = 50  # valor fijo de ejemplo

# ---- ESTADO DE LA APP ----
if "intensidad" not in st.session_state:
    st.session_state.intensidad = 50
...
# Mostrar porcentaje de luz detectada
...
with col5:
    if st.button("🎨 RANDOM") and st.session_state.encendido:
...
with col6:
    if st.button("AUTO") and st.session_state.encendido:
...
st.write(f"🎨 **Color actual:** {st.session_state.color if st.session_state.encendido else '-----'}")
st.write(f"🤖 **Modo automático:** {'Activado' if st.session_state.modo_automatico and st.session_state.encendido else 'Desactivado'}")

# ---- BOTÓN PARA SELECCIÓN MANUAL DE COLOR ----
if st.button("🎨") and st.session_state.encendido:
    st.switch_page("seleccion_color")
